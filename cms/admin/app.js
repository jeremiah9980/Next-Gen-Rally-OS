/* NextGen CMS admin dashboard.
   Loads cms/content/nextgen-site.json, lets a coach/board member edit every
   section through plain forms, validates against the schema rules, and
   exports an updated JSON file to replace cms/content/nextgen-site.json
...
function exportJson() {
  ...
  a.download = "nextgen-site.json";
  document.body.appendChild(a);
  a.click();
  a.remove();
  showStatus((ok ? "Exported" : "Exported with warnings") + " — replace cms/content/nextgen-site.json with this file, then commit & push.", ok ? "ok" : "error");
}
...
function init() {
  ...
  fetch("../content/nextgen-site.json")
    ...
    .catch(function (err) {
      showStatus("Could not load cms/content/nextgen-site.json. Serve this folder over HTTP (e.g. npx serve .) rather than opening it from disk.", "error", String(err));
    });
}
